//Timing Observations:
//We tested the timing of the mutex in Part 1 and the spinlock in Part 2 with 100 threads, as an example:
//spinlock ran in less time than the mutex from part 1 (4.793283 (mutex) - 4.539672 (spinlock) = 0.253611)
//This makes sense because when a thread is blocked on a mutex there is overhead related to switching to another thread and then returning.
//The spinlock thread busy-waits until the lock is available, so there is no overhead related to switching out threads.
